export const projectsData = [
  {
    id: 1,
    title: "Doryanne",
    subtitle: "Thérapeute holistique, praticienne et enseignante énergétique ",
    
    infos:
      "J'ai pour passion d'enseigner mon métiers à des personnes désirant s'élever spirituellement et désirant changer leur vies avec les thérapies énergétiques. Cette plateforme e-learning vous donneras les clefs en mains pour vous faire des auto traitement mais également des soins énergétiques sur vos prochains consultant. Que vous soyez débutant ou un professionnel déjà établi, la formation est pour tout le monde car je vous donne tous mes connaissances pour avoir les clefs en main afin d'etre en confiance dans votre pratique. Ici vous retrouverez mes initiations énergétique avec les différents maître ascensionnés avec lequel je travaille au quotidien.",
    img: "./assets/img/capt dodo-site.png",
    link: "https://doryanne-therapeuteholistique.com/",
  },
  {
    id: 2,
    title: "La Guidance",
   subtitle: "Plateforme holistique",
    
    infos:
      "La guidance est une Plateforme de prise de rendez-vous Holistique et vente de produits artisanaux ésoterique. Le but premier de cette plateforme est le partage et le professionnalisme, en sélectionnant soigneusement chaque praticien énergétique, medium ou astrologue, nous favorisons une vibration qui se dirige toujours dans la même direction, accompagnement bienveillant, sérieux et dans un seul but : votre élévation personnel. Vous retrouverez également une partie boutique de produit ésoterique afin de vous aider au quotidien.",
    img: "./assets/img/guidabce.jpg",
    link: "https://www.instagram.com/laguidance/",
  },
  {
    id: 3,
    title: "Linktree",
    subtitle: "Mes divers liens",
    
    infos:
      "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quas cumque labore suscipit, pariatur laboriosam autem omnis saepe quisquam enim iste.",
    img: "./assets/img/linktree.png",
    link: "https://linktr.ee/Doryanne",
  },
  /*{
    id: 4,
    title: "Creative Dev",
    date: "Juillet 2020",
    languages: ["Vue", "Php"],
    infos:
      "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quas cumque labore suscipit, pariatur laboriosam autem omnis saepe quisquam enim iste.",
    img: "./assets/img/projet-4.jpg",
    link: "http://www.google.com",
  },*/
];
